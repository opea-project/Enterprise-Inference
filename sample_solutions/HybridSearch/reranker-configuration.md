...
```bash
# TOKEN: the litellm_master_key from core/inventory/metadata/vault.yml
# (generated by generate-vault-secrets.sh — not a Keycloak token)
TOKEN="your-vault-token-here"

# BASE_URL: GenAI Gateway base URL (no /v1 suffix)
# Used for both admin endpoints (/model/info, /model/update) and inference (/v1/rerank)
...
> **Note:** Use the `"documents"` field for GenAI Gateway (LiteLLM/Cohere provider). For Keycloak/APISIX deployments, use `"texts"` instead — the two backends expect different field names.
...
**Expected successful response (GenAI Gateway — LiteLLM/Cohere format):**
...
> **Note:** For Keycloak/APISIX deployments, the response is a flat array format instead:
> ```json
> [{"index": 2, "score": 0.9274}, {"index": 0, "score": 0.9241}, {"index": 1, "score": 0.0103}]
> ```
