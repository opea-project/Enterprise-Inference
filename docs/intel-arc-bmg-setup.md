...
#### 1. Enterprise inference prerequisites: [docs/prerequisites.md](prerequisites.md)
...
> **Driver note (`xe` vs `i915`)**: Newer stacks (e.g. Ubuntu 25.10 on Arc B-series) expose the resource as `gpu.intel.com/xe`, which is the default. Hosts still on the legacy `i915` kernel driver expose `gpu.intel.com/i915` instead. In that case, override the resource name in `core/helm-charts/vllm/bmg-values.yaml`:
>
> ```yaml
> xpuDeviceResource: "gpu.intel.com/i915"
> ```
...
- **Block size**: 64 (required for IPEX chunked prefill; optimized for XPU KV cache)
