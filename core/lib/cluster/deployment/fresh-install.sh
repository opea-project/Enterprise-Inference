    ...
    # Check if all deployment steps are set to "no" after getting user input
        ...
        if [[ "$proceed_with_installation" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
            ...
            if [[ "$deploy_agenticai_plugin" == "yes" ]]; then
                ...
                if ansible-playbook -i "${INVENTORY_PATH}" ../../plugins/agenticai/playbooks/deploy-agenticai-plugin.yml \
                    --extra-vars "cluster_url=${cluster_url} \
                                  cert_file=${cert_file} \
                                  key_file=${key_file} \
                                  kubernetes_platform=${kubernetes_platform}" \
                    --vault-password-file "$vault_pass_file"; then
            ...
            if [[ "$deploy_finetune_plugin" == "yes" ]]; then
                ...
                if ansible-playbook -i "${INVENTORY_PATH}" ../../blueprints/finetuning_service/playbooks/deploy-all.yml \
                    --extra-vars "cluster_url=${cluster_url} \
                                  cert_file=${cert_file} \
                                  key_file=${key_file} \
                                  kubernetes_platform=${kubernetes_platform}" \
