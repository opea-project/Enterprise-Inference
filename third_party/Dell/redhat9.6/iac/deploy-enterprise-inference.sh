...
# --- RHEL Kubernetes Preflight Steps ---
echo "[INFO] Running RHEL Kubernetes preflight configuration..."
...
# Ensure local-path-provisioner directory exists with correct permissions
...
if [[ $(readlink -f /usr/bin/python3) != "/usr/bin/python3.11" ]]; then
    log_info "Updating /usr/bin/python3 symlink to python3.11..."
    sudo ln -sf /usr/bin/python3.11 /usr/bin/python3
    log_success "/usr/bin/python3 now points to python3.11"
fi
...
# ------------------------------------------------------------
# Repository & Subscription Fix Helpers
# ------------------------------------------------------------
sanitize_repos() {
    log_info "Sanitizing DNF repositories..."

    # Disable Habana repos (they break on RHEL 9.7)
    if ls /etc/yum.repos.d/habanalabs_*.repo >/dev/null 2>&1; then
        log_warn "Disabling Habana repositories..."
        for repo in /etc/yum.repos.d/habanalabs_*.repo; do
            sed -i 's/^enabled=.*/enabled=0/' "$repo" || true
        done
    fi

    # Prevent repo failures from breaking DNF
    if ! grep -q "^skip_if_unavailable=true" /etc/dnf/dnf.conf 2>/dev/null; then
...
repair_subscription_manager() {
    log_info "Ensuring subscription-manager uses platform-python..."

    if [ -f /sbin/subscription-manager ]; then
        first_line=$(head -1 /sbin/subscription-manager)
        if echo "$first_line" | grep -q "/usr/bin/python"; then
            sed -i '1s|.*|#!/usr/libexec/platform-python|' /sbin/subscription-manager
            log_warn "subscription-manager shebang corrected to platform-python"
        fi
    fi
}
...
main() {
    ...
    if ! skip_if_completed "system_packages"; then
        ...
        log_info "=========================================="
        log_info "Step 1: Preparing System & Installing Packages"
        log_info "=========================================="
        ...
        # Install libselinux-python3 safely
        dnf install -y python3-libselinux policycoreutils-python-utils || {
        log_warn "SELinux python bindings install skipped (may already exist)"
      }

        # Install dnf plugins (required for config-manager)
        dnf install -y dnf-plugins-core || true
        
        if ! command -v /usr/local/bin/python3 >/dev/null 2>&1; then
            ln -sf /usr/bin/python3.11 /usr/local/bin/python3
            log_info "python3 now resolves to python3.11 via /usr/local/bin override"
        fi
        if [[ "$(readlink -f /usr/local/bin/python3)" != "/usr/bin/python3.11" ]]; then
            log_error "python3 symlink not pointing to 3.11"
            exit 1
        fi
        # Enable CRB repo (RHEL 9)
        dnf config-manager --set-enabled crb 2>/dev/null || true

        log_info "python3 now points to: $(python3 --version)"
            
        # Validate python
        if ! /usr/bin/python3.11 -c 'import sys; exit(0 if sys.version_info >= (3,10) else 1)'; then
    ...
    if ! skip_if_completed "deploy_stack"; then
        export python3_interpreter=/usr/bin/python3.11
        ...
        log_info "Configuring sudo NOPASSWD and secure_path for ${USERNAME}..."

        SUDO_FILE="/etc/sudoers.d/${USERNAME}"

        cat > "${SUDO_FILE}" <<EOF
${USERNAME} ALL=(ALL) NOPASSWD: ALL
Defaults:${USERNAME} secure_path=/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
EOF

        chmod 440 "${SUDO_FILE}"

        if visudo -cf "${SUDO_FILE}" >/dev/null 2>&1; then
            log_success "Sudo NOPASSWD and secure_path configured for ${USERNAME}"
        else
	    ...
	    if [[ -f "$HOSTS_FILE" ]]; then
          ...
          if grep -q "ansible_python_interpreter:" "$HOSTS_FILE"; then
              sed -i -E "s|ansible_python_interpreter:.*|ansible_python_interpreter: /usr/bin/python3.9|" "$HOSTS_FILE"
          else
              sed -i "/ansible_user:/a\      ansible_python_interpreter: /usr/bin/python3.9" "$HOSTS_FILE"
          fi     
      else
        ...
        log_info "Ensuring python 3.11 has pip..."
        if ! /usr/bin/python3.11 -m pip --version >/dev/null 2>&1; then
            log_warn "Bootstrapping pip for python 3.11..."
            /usr/bin/python3.11 -m ensurepip --upgrade || true
        fi
