...
if [[ -f "$STATE_FILE" ]] || [[ "$RESUME" == true ]]; then
    if [[ -f "$STATE_FILE" ]]; then
        ...
        source "$STATE_FILE"
        RESUME=true
        # Re-apply deployment variables based on saved deployment mode
        set_deployment_variables
    else
...
log_warn "Have you verified that your Hugging Face token has access to the model(s): ${MODELS} ?"
if [[ -t 0 ]]; then
    read -r -p "Type 'yes' to continue or anything else to exit: " HF_CONFIRM
else
    log_warn "Non-interactive shell detected, auto-continuing Hugging Face check"
    HF_CONFIRM="yes"
fi

if [[ "${HF_CONFIRM}" != "yes" ]]; then
    log_error "Please verify Hugging Face model access before running the deployment."
    exit 1
fi
...
main() {
    ...
    if ! skip_if_completed "clone_repo"; then
	    ...
	    su "${USERNAME}" -c "cd /home/${USERNAME}/Enterprise-Inference && git checkout ${BRANCH}"
        ...
        if [[ "$GPU_TYPE" == "cpu" ]] || [[ "$GPU_TYPE" == "gaudi3" ]]; then
            CONFIG_FILE="/home/${USERNAME}/Enterprise-Inference/core/inventory/inference-config.cfg"
                if [[ -f "$CONFIG_FILE" ]]; then
                        ...
                        "$CONFIG_FILE"
                        log_info "Updated inference-config.cfg with models='${MODELS}' and cpu_or_gpu=cpu"
                else
